...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console v4.4.3")
    ...
    # ==========================================
    # v4.4.2 UI Automator Download Prompt Check
    # ==========================================
    def _check_and_click_download_prompt(self, serial, run_log_file):
        try:
            ...
            prompt_keywords = [
                ...
                "Accept & continue", "No thanks", "接受並繼續", "不用了", "接受并继续", "Allow", "允許", "允许"
            ]
            
            if any(k in xml_dump for k in prompt_keywords):
                self.ui_log("   ... [UI Alert] Detected Chrome Prompt/FRE. Attempting to bypass...", serial, run_log_file)
                
                # 擴充正則表達式，涵蓋 FRE 與權限按鈕
                click_targets = "Download|仍要|下載|下载|Accept|No thanks|不用了|接受|繼續|继续|Allow|允許|允许"
                pattern = r'<node[^>]*text="([^"]*(' + click_targets + r')[^"]*)"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"[^>]*>'
                ...
                for match in matches:
                    ...
                    # 排除標題文字與 "Downloads" 目錄路徑，避免誤點資料夾按鈕
                    if "Choose" in text_val or "選擇" in text_val or "选择" in text_val or text_val.strip().lower() == "downloads":
                        continue
                ...
                # 備用機制：透過 ID 盲點
                if not clicked:
                    id_pattern = r'<node[^>]*resource-id="([^"]*(terms_accept|negative_button|button_primary|button_secondary)[^"]*)"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"[^>]*>'
                    id_matches = re.finditer(id_pattern, xml_dump, re.IGNORECASE)
                    for match in id_matches:
                        x1, y1, x2, y2 = map(int, match.group(3, 4, 5, 6))
                        cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
                        if cx > 0 and cy > 0:
                            self.ui_log(f"   ... Clicking resource-id button at ({cx}, {cy})", serial, run_log_file)
                            self.run_adb(["shell", "input", "tap", str(cx), str(cy)], serial=serial, capture=False)
                            clicked = True
                            time.sleep(1.5)
                            break
                ...
                if not clicked:
                    self.ui_log("   ... Exact button bounds not found, using fallback keys (TAB -> ENTER).", serial, run_log_file)
                    self.run_adb(["shell", "input", "keyevent", "61"], serial=serial, capture=False)
                    time.sleep(0.5)
                    self.run_adb(["shell", "input", "keyevent", "66"], serial=serial, capture=False)
            ...
            if test_type == "[APM] Power & Display (Wake-up & Brightness)":
                ...
                for i in range(1, target_val + 1):
                    ...
                    while time.time() - start_time < dl_timeout:
                        ...
                        # 加快偵測頻率以快速通過多頁 Chrome FRE
                        if current_size == 0 and (time.time() - start_time) > 4:
                            if (time.time() - last_ui_check_time) > 4:
                ...
                for line in out_users.splitlines():
                    if "MDM_Stress" in line:
                        try:
                            ...
                            self.run_adb(["shell", "am", "stop-user", "-f", uid], serial=serial) # 先強制停止
                            self.run_adb(["shell", "pm", "remove-user", uid], serial=serial)
                            time.sleep(4) # 等待系統非同步刪除
                        except: pass
                ...
                for i in range(1, target_val + 1):
                    ...
                    # Auto-recovery for system_server soft reboot (OOM crash)
                    if "Can't find service" in out or "error: closed" in out or "DeadSystemException" in out or "Exception" in out or "error" in out.lower() or not out.strip():
                        self.ui_log(f"🚨 [OOM/Crash] system_server died! Waiting for Soft Reboot (up to 120s)...", serial, run_log_file)
                        ...
                        for wait_sec in range(0, 120, 5):
                            ...
                            if ":" in serial:
                                try:
                                    subprocess.run(["adb", "connect", serial], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3, **get_cflags())
                                except: pass
                            
                            check = self.run_adb(["shell", "pm", "list", "users"], serial=serial, timeout=5)
                            if "Users:" in check:
                                self.ui_log(f"🔄 System recovered after {wait_sec+5}s. Retrying...", serial, run_log_file)
                                break
                    ...
                    # 加入 -w 參數，強制等待 User 啟動完成，避免後續指派權限失敗
                    self.run_adb(["shell", "am", "start-user", "-w", user_id], serial=serial)
                    time.sleep(3)
                    ...
                    self.run_adb(["shell", "am", "stop-user", "-f", user_id], serial=serial)
                    time.sleep(3)
                    self.run_adb(["shell", "pm", "remove-user", user_id], serial=serial)
                    self.ui_log(f"⏳ Waiting 15s for system_server memory GC and I/O cleanup...", serial, run_log_file)
                    for _ in range(15):
                        if self.device_stop_event.get(serial, False): break
                        time.sleep(1)
                    completed = i
            ...
            if test_type in ["Background Download Stress (curl/wget)"]:
                ...
                for line in out.splitlines():
                    if "MDM_Stress" in line:
                        try:
                            uid = line.split("{")[1].split(":")[0].strip()
                            self.run_adb(["shell", "am", "stop-user", "-f", uid], serial=serial, capture=False)
