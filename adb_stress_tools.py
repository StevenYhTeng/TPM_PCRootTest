...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console v4.3.3")
    ...
    def dismiss_camera_prompts(self, serial):
        """自動繞過 Camera 初次使用的地理位置確認彈窗"""
        # 點擊畫面右下方(Next/OK)與正下方，並送出 Enter 鍵
    ...
    def dismiss_chrome_prompts(self, serial):
        """自動繞過 Chrome 首次啟動通知對話框與設定精靈"""
        # 強制關閉可能攔截畫面的 Google Setup Wizard
        self.run_adb(["shell", "am", "force-stop", "com.google.android.setupwizard"], serial=serial, capture=False)
        # 模擬 Tab 切換按鈕並按下 Enter，通常可跳過「No Thanks」或「Continue」
        for _ in range(3):
    ...
    def _run_test_thread(self, test_type, target_val, timestamp, base_test_name, unit, serial, kw, sleep_sec, wake_sec, dl_url, dl_delete_after, dl_file, dl_timeout, oom_pct, oom_mins, reboot_up, reboot_down, do_shutdown):
        ...
        try:
            ...
            if test_type == "[APM] Power & Display (Wake-up & Brightness)":
                ...
                # 🌟 修復 1: 複合式 APM 相機壓力測試 (一次包含 4 種測試場景)
                ...
                for i in range(1, target_val + 1):
                    ...
                    time.sleep(4)
                    self.dismiss_camera_prompts(serial) # 🌟 繞過提示框
                ...
                # 🌟 修復 5: 防止 Google Setup Wizard 或 Chrome 帳號綁定頁面卡住 (Checking Info)
                ...
                for i in range(1, target_val + 1):
                    ...
                    # 執行繞過腳本
                    self.dismiss_chrome_prompts(serial)
                ...
                for i in range(1, target_val + 1):
                    ...
                    # 🌟 修復 2: 解決無反應問題，改用最泛用的 Intent 並配合連續的 Media 廣播
                    ...
                    # 兩次 Enter 嘗試跳過 "Just once" (僅限一次) 對話框
                    for _ in range(2):
                        self.run_adb(["shell", "input", "keyevent", "66"], serial=serial, capture=False) 
                        time.sleep(1)
                    
                    # 移除容易導致暫停的 tap 中間座標，改用直接注入 PLAY 鍵
                    for _ in range(3):
                ...
                for i in range(1, target_val + 1):
                    ...
                    self.run_adb(["shell", "input", "keyevent", "223"], serial=serial) 
                    time.sleep(2)
                    self.run_adb(["shell", "input", "keyevent", "126"], serial=serial, capture=False) # 確保鎖屏後仍在播放
                ...
                for idx, apk_file in enumerate(apks):
                    ...
                    try:
                        ...
                        file_size_mb = 0.0
                    self.ui_log(f"   ... Copying [{idx+1}/{total_apks}]: {apk_file} ({file_size_mb:.2f} MB)", serial, run_log_file)
                    
                    start_push_time = time.time()
                    self.run_adb(["push", local_path, f"{remote_apk_dir}/{apk_file}"], serial=serial)
                    push_elapsed = time.time() - start_push_time
                    self.ui_log(f"   ✅ Copied in {push_elapsed:.1f}s", serial, run_log_file)
                
                if self.device_stop_event.get(serial, False):
                    raise Exception("APK Transfer aborted by user.")
                ...
                for i in range(1, target_val + 1):
                    ...
                    for s in range(target_seconds):
                        ...
                        # 🌟 修復 4: 確保短影片不會播完就暫停，每 60 秒強制送一次播放指令
                        if s > 0 and s % 60 == 0:
                            self.run_adb(["shell", "input", "keyevent", "126"], serial=serial, capture=False) # KEYCODE_MEDIA_PLAY
