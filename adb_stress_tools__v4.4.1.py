...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console v4.4.1")
    ...
    # ==========================================
    # v4.4.0 UI Automator Download Prompt Check
    # ==========================================
        ...
        try:
            ...
            if test_type == "[APM] Power & Display (Wake-up & Brightness)":
                ...
                for i in range(1, target_val + 1):
                    ...
                    # Auto-recovery for system_server crash
                    if "Can't find service" in out or "error: closed" in out or "DeadSystemException" in out:
                        self.ui_log(f"⚠️ System busy (system_server crash detected). Waiting 20s for auto-recovery...", serial, run_log_file)
                        time.sleep(20)
                        if self.device_stop_event.get(serial, False): break
                        out = self.run_adb(["shell", "pm", "create-user", "--profileOf", "0", "--managed", "MDM_Stress"], serial=serial)
                    ...
                    self.ui_log(f"⏳ Waiting 12s for system_server to completely wipe user data...", serial, run_log_file)
                    for _ in range(12):
                        if self.device_stop_event.get(serial, False): break
                        time.sleep(1)
                    completed = i
