...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console (TPM/APM Edition) v3.9.5")
        ...
        # 🌟 Prevent Windows from sleeping while the app is open
        prevent_system_sleep()
        
        # 🌟 GDPR Compliance Check BEFORE setting up UI/Timers to prevent background deadlock
    ...
    def _bg_auto_connect(self, prefixes, start, end):
        ...
        def connect_ip(ip):
            cmd = ["adb", "connect", f"{ip}:5555"]
            try:
                # 🌟 No shell=True. This allows proper timeout killing of the adb.exe process!
                subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=2, **get_cflags())
            except Exception:
                pass
        ...
        # 🌟 Reduced from 200 to 50 to prevent ADB Daemon DDoS freeze
    ...
    def on_closing(self):
        ...
        # 🌟 Allow system sleep again before closing
        allow_system_sleep()
        self.root.destroy()
    ...
    def run_adb(self, cmd_list, serial=None, capture=True):
        try:
            ...
            res = subprocess.run(cmd, capture_output=capture, text=True, timeout=15, **get_cflags())
    ...
    def _run_test_thread(self, test_type, target_val, timestamp, base_test_name, unit, serial, pkgs_str, throttle_val, ignore_crash_val, sleep_sec, wake_sec, dl_url, dl_delete_after, dl_file, dl_timeout, oom_pct, oom_mins, reboot_up, reboot_down, do_shutdown):
        ...
        try:
            ...
            # 🌟 Properly managed logcat process without leaking shell instances
            ...
            if test_type == "Fingerprint HAL & Screen Wake Stress":
                ...
                proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', **get_cflags())
            ...
            # 🌟 Properly close logcat processes to prevent leaking handles
