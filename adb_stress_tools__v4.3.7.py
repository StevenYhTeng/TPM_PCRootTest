...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console v4.3.7")
            ...
            if test_type == "[APM] Power & Display (Wake-up & Brightness)":
                ...
                for i in range(1, target_val + 1):
                    ...
                    while time.time() - start_time < dl_timeout:
                        ...
                        if current_size == 0 and not prompt_clicked and (time.time() - start_time) > 8:
                            # 經過 8 秒如果還沒開始下載，盲按 TAB + ENTER 來確認「危險檔案下載」提示
                            self.ui_log("   ... Attempting to auto-click 'Download Anyway' prompt...", serial, run_log_file)
                            for _ in range(2):
