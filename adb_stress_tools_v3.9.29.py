...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console v3.9.29")
    ...
    def setup_ui(self):
        ...
        self.test_type_var = tk.StringVar(value="Video Streaming Stress Test")
        test_options = [
        ...
        # 🌟 App & Monkey 設定面板 (新增抓取 System Apps 功能)
        ...
        self.chk_ignore_crash = tk.Checkbutton(self.app_frame, text="Ignore Crash (Monkey & OOM)", variable=self.ignore_crash_var)
        self.chk_ignore_crash.pack(anchor=tk.W)
        
        self.ignore_anr_var = tk.BooleanVar(value=True)
        self.chk_ignore_anr = tk.Checkbutton(self.app_frame, text="Ignore ANR / Timeouts (Monkey & OOM)", variable=self.ignore_anr_var)
        self.chk_ignore_anr.pack(anchor=tk.W)
        
        # 🌟 Storage I/O 設定面板 (新增多路徑抓取與並發控制)
        ...
        self.entry_storage_paths = tk.Entry(self.storage_frame, font=("Arial", 10))
        self.entry_storage_paths.insert(0, "/data/local/tmp")
        self.entry_storage_paths.pack(fill=tk.X, pady=(0, 5))
    ...
    def on_test_type_changed(self, event):
        ...
        needs_video = "Video Streaming" in test_type
        
        for f in [self.dl_frame, self.app_frame, self.screen_frame, self.oom_frame, self.reboot_frame, self.mdm_frame, self.video_frame, self.storage_frame]: 
            f.pack_forget()
        ...
        if "Storage I/O" in test_type:
            self.storage_frame.pack(fill=tk.X, pady=(0, 10))
    ...
    def _bg_fetch_apps(self, serial):
        # 🌟 根據 Include System Apps 決定是否加上 -3 參數
        cmd = ["shell", "pm", "list", "packages"]
        if not self.include_sys_apps_var.get(): 
            cmd.append("-3")
    ...
    def _show_app_selector(self, pkgs):
        self.btn_fetch_apps.config(state=tk.NORMAL)
        if not pkgs:
            messagebox.showwarning("Warning", "No apps found.")
            return
    ...
    def fetch_storages_ui(self):
        selections = self.device_listbox.curselection()
        if not selections or "No devices" in self.device_listbox.get(selections[0]):
            messagebox.showerror("Error", "Please select at least one device to fetch storages from.")
            return
    ...
    def _bg_fetch_storages(self, serial):
        ...
        for line in out.splitlines()[1:]:
            ...
            if len(parts) >= 6:
                mount = parts[5]
                # 過濾掉明顯是系統或虛擬唯讀的磁區，只留下可能可寫入的實體路徑
                if not mount.startswith(('/apex', '/sys', '/proc', '/dev', '/vendor', '/system', '/linkerconfig')):
                    if mount not in mounts: 
                        mounts.append(mount)
                        
        if not mounts: 
            mounts = ["/data/local/tmp", "/sdcard"]
            
        def _ui():
            top = tk.Toplevel(self.root)
            top.title("Select Storage Mounts")
    ...
    def start_test(self):
        ...
        if test_type == "Video Streaming Stress Test":
            try:
                target_str = self.entry_vid_cycles.get().strip()
                target_val = int(target_str)
                if target_val <= 0: raise ValueError
            except:
                messagebox.showerror("Error", "Please enter a valid number of Cycles for Video Streaming!")
                return
        else:
            target_str = self.entry_target.get().strip()
            if not target_str.isdigit() or int(target_str) <= 0:
                messagebox.showerror("Error", "Please enter a valid target number greater than 0!")
                return
            target_val = int(target_str)
        ...
        if test_type == "Video Streaming Stress Test":
            try: v_hours = float(self.entry_vid_hours.get().strip())
            except: v_hours = 2.0
            try: v_pause = float(self.entry_vid_pause.get().strip())
            except: v_pause = 5.0
            
            kwargs["vid_url"] = self.entry_vid_url.get().strip()
            kwargs["vid_hours"] = v_hours
            kwargs["vid_pause"] = v_pause
        ...
        type_map = {
            "Video Streaming Stress Test": "Video_Stream",
    ...
    def _run_test_thread(self, test_type, target_val, timestamp, base_test_name, unit, serial, kw, sleep_sec, wake_sec, dl_url, dl_delete_after, dl_file, dl_timeout, oom_pct, oom_mins, reboot_up, reboot_down, do_shutdown):
        ...
        try:
            ...
            # 🌟 OSD Watermark - 配合 v7.0 APK 使用 || 作為換行符號
            ...
            if test_type == "Video Streaming Stress Test":
                for i in range(1, target_val + 1):
                    ...
                    self.ui_log(f"--- Cycle {i}/{target_val} : Wake up & Start Video Stream ---", serial, run_log_file)
                    ...
                    url = kw.get("vid_url", "")
                    play_seconds = int(kw.get("vid_hours", 2.0) * 3600)
                    
                    self.ui_log(f"🔗 Launching URL: {url}", serial, run_log_file)
                    
                    if "youtu" in url.lower():
                        self.ui_log("🎬 YouTube URL detected. Opening via Browser/App...", serial, run_log_file)
                        self.run_adb(["shell", "am", "start", "-a", "android.intent.action.VIEW", "-d", f'"{url}"'], serial=serial)
                        self.ui_log("⏳ Waiting 8s for YouTube page to load...", serial, run_log_file)
                        time.sleep(8)
                        
                        self.run_adb(["shell", "input", "tap", "500", "500"], serial=serial)
                        self.run_adb(["shell", "input", "tap", "500", "500"], serial=serial)
                        time.sleep(1)
                        
                        self.ui_log("🔄 Sending 'F' key to trigger native YouTube Full-Screen...", serial, run_log_file)
                        self.run_adb(["shell", "input", "keyevent", "34"], serial=serial)
                        
                    else:
                        self.ui_log("🎬 Using Native Video Player intent for guaranteed Full-Screen...", serial, run_log_file)
                        self.run_adb(["shell", "am", "start", "-a", "android.intent.action.VIEW", "-d", f'"{url}"', "-t", "video/*"], serial=serial)
                        self.ui_log("⏳ Waiting 5s for player to buffer...", serial, run_log_file)
                        time.sleep(5)
                        self.run_adb(["shell", "input", "keyevent", "126"], serial=serial)

                    self.ui_log(f"▶️ Streaming for {kw.get('vid_hours')} hours ({play_seconds} seconds)...", serial, run_log_file)
                    start_time = time.time()
                    
                    while time.time() - start_time < play_seconds:
                        if not self.device_testing_state.get(serial, False): break
                        
                        elapsed = int(time.time() - start_time)
                        if elapsed > 0 and elapsed % 300 == 0:
                            self.ui_log("   ... Sending Keep-Awake signal (KeyEvent 224)", serial, run_log_file)
                            self.run_adb(["shell", "input", "keyevent", "224"], serial=serial)
                    ...
                    self.ui_log("⏹️ Cycle finished. Stopping playback...", serial, run_log_file)
                    self.run_adb(["shell", "am", "force-stop", "com.android.chrome"], serial=serial)
                    self.run_adb(["shell", "am", "force-stop", "com.google.android.youtube"], serial=serial)
                    self.run_adb(["shell", "am", "force-stop", "com.google.android.apps.photos"], serial=serial)
                    
                    self.run_adb(["shell", "input", "keyevent", "3"], serial=serial)
                    
                    pause_seconds = int(kw.get("vid_pause", 5.0) * 60)
                    if pause_seconds > 0 and i < target_val:
                        self.ui_log(f"⏸️ Cooling down for {kw.get('vid_pause')} minutes ({pause_seconds}s)...", serial, run_log_file)
                        for sec in range(pause_seconds):
                            if not self.device_testing_state.get(serial, False): break
                            time.sleep(1)
                            
                    completed = i
            
            elif test_type == "Fingerprint HAL & Screen Wake Stress":
                ...
                while m_proc.poll() is None:
                    if not self.device_testing_state.get(serial, False) or time.time() - start_time >= target_sec: break
                    time.sleep(1)
