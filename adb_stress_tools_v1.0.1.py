...
class ADBStressGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Android ADB Stress Test Console (Pro Edition) v1.0.0")
    ...
    def setup_ui(self):
        ...
        # --- Screen ON/OFF Specific Settings (Dynamic Frame) ---
        ...
        self.entry_sleep_time.insert(0, "10") # Default 10 seconds
        self.entry_sleep_time.pack(fill=tk.X, pady=(0, 5))

        # --- Buttons Area ---
        self.btn_frame = tk.Frame(control_frame)
        self.btn_frame.pack(fill=tk.X, pady=20)

        self.btn_start = tk.Button(self.btn_frame, text="▶️ START", font=("Arial", 12, "bold"), bg="#4CAF50", fg="white", command=self.start_test)
        self.btn_start.pack(fill=tk.X, pady=(0, 10))

        self.btn_stop = tk.Button(self.btn_frame, text="⏹️ STOP", font=("Arial", 12, "bold"), bg="#F44336", fg="white", state=tk.DISABLED, command=self.stop_test)
    ...
    def on_test_type_changed(self, event):
        ...
        if needs_app:
            self.app_frame.pack(fill=tk.X, pady=(0, 15), before=self.btn_frame)
        ...
        if needs_screen:
            self.screen_frame.pack(fill=tk.X, pady=(0, 15), before=self.btn_frame)
        else:
    ...
    def _run_test_thread(self, test_type, target_val, timestamp):
        ...
        try:
            ...
            if test_type == "WiFi ON/OFF Test":
                ...
                try:
                    sleep_sec = int(self.entry_sleep_time.get().strip())
                except ValueError:
                    sleep_sec = 10 # Fallback default
                ...
                for i in range(1, target_val + 1):
                    ...
                    # Wait dynamically to allow rapid stop
                    self.ui_log(f"Sleeping for {sleep_sec} seconds...")
            ...
            # All tests successfully finished without exceptions
            status = "PASS"
